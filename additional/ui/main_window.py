...
from additional.core.text_processor import TextProcessor
from additional.core.knowledge_base import KnowledgeBase
from ui.horizontal_line import HorizontalLine
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # TODO: Исправить ширину столбцов таблицы
        ...
        self.btn_new_domain.clicked.connect(self.new_domain)
        ...
        # Статус бар
        self.statusBar().showMessage('В работе')
    ...
    def create_menu_bar(self):
        ...
        # Меню "База данных"
        data_base_menu = menubar.addMenu('База данных')

        export_action = QAction('Экспорт', self)
        export_action.triggered.connect(self.export_data)
        data_base_menu.addAction(export_action)

        import_action = QAction('Импорт', self)
        import_action.triggered.connect(self.import_data)
        data_base_menu.addAction(import_action)
    ...
    def analyze_text(self):
        ...
        if not self.knowledge_base:
            self.knowledge_base = KnowledgeBase("Новая область")
        ...
        result_text += f"Обнаружено возможных правил: {len(rules)}\n"
        result_text += f"Обнаружено фактов: {len(facts)}\n\n"
        ...
        self.statusBar().showMessage(f'Анализ завершен. Обнаружено {len(rules)} правил и {len(facts)} фактов')
    ...
    def view_rules(self):
        try:
            ...
            self.statusBar().showMessage(f'Отображено {len(self.rules)} правил')
        ...
        try:
            ...
            if ok and agent_id:
                ...
                if selected_agent_id:
                    # Здесь будет вызов метода трассировки. Для демонстрации создаем тестовый вывод
                    ...
                    # Ищем правила агента
                    agent_rules = [r for r in self.rules if r['agent'] == selected_agent_id]
        ...
        try:
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                for agent_name in selected_agents:
                    ...
                    if agent_id:
                        agent_rules = [r for r in self.rules if r['agent'] == agent_id]
    ...
    def forward_inference(self):
        """Прямой вывод"""
        try:
            # Здесь будет вызов InferenceEngine
            # Для демонстрации создаем тестовый вывод
    ...
    # Формат выходного файла БД: MSAccess или csv-файл
